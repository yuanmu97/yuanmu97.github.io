---
id: "2RZFjJmyLa5QjnJuVpEB"
title: "从资源效率分析：人类负责 aha moment，机器负责中间推演"
date: "2026-03-22T04:31:14.342Z"
published: true
---
...
如果我们将整个过程抽象为 \((t_0, t_1, t_2, \dots)\) 这些节点，以深度学习的发展为例，可以对应到如反向传播、ResNet、Transformer、o1 等关键节点，而其间伴随着的是上百万篇论文、无数模型变体与工程实现。那么，一个自然的问题是：仅从这些关键节点的产生来看，不同计算系统的成本如何？
...
而在当前主流的自回归 AI 范式中，情况恰恰相反。模型的生成过程依赖其全部历史上下文，即 \(x_t = f(x_1, x_2, \dots, x_{t-1})\)。这意味着，随着推理持续进行，系统必须维护一个不断增长的状态。即便通过 KV cache 降低重复计算，其内存与带宽成本仍然与上下文长度近似线性相关。在一个持续运行的推理或 agent 系统中，随着时间推进，token 数量、状态规模与整体开销都会不断累积。因此，一个发生在更晚时间点的关键结果，其产生往往依赖于一个更长、更昂贵的显式历史。从这个角度看，AI 的 “aha moment” 并不是在恒定成本下产生的，而是嵌入在一个持续增长的上下文结构之中。
